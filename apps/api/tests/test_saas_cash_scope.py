    ...
    with cash_scope_session as session:
        ...
        assert (
            session.scalar(
                ...
                .select_from(models.user_cash_cut_commands)
                .where(models.user_cash_cut_commands.c.idempotency_key == "cut-a")
            )
            == 2
        )
        ...
        commands_before = session.scalar(
            sa.select(sa.func.count())
            .select_from(models.user_cash_cut_commands)
            .where(models.user_cash_cut_commands.c.organization_id == org_b)
        )
...
def test_reporting_expenses_uses_actor_tenant_not_pilot_organization(
    ...
    with cash_scope_session as session:
        tenant_a = signup_tenant(
            session,
            {
                "business_name": "Reportes A",
                "owner_name": "Owner A",
                "email": "reports-a@example.test",
                "password": "test-only-reports-a",
                "business_type": "taqueria",
            },
        )
        tenant_b = signup_tenant(
            session,
            {
                "business_name": "Reportes B",
                "owner_name": "Owner B",
                "email": "reports-b@example.test",
                "password": "test-only-reports-b",
                "business_type": "taqueria",
            },
        )
        org_a, branch_a, owner_a = (
            tenant_a["organization"]["id"],
            tenant_a["branch"]["id"],
            tenant_a["user"]["id"],
        )
...
def test_cash_concepts_are_isolated_by_actor_tenant_and_idempotency(
    ...
    with cash_scope_session as session:
        tenant_a = signup_tenant(
            session,
            {
                "business_name": "Conceptos A",
                "owner_name": "Owner A",
                "email": "concepts-a@example.test",
                "password": "test-only-concepts-a",
                "business_type": "taqueria",
            },
        )
        tenant_b = signup_tenant(
            session,
            {
                "business_name": "Conceptos B",
                "owner_name": "Owner B",
                "email": "concepts-b@example.test",
                "password": "test-only-concepts-b",
                "business_type": "taqueria",
            },
        )
...
def test_cash_movements_scope_concepts_commands_and_ledger_by_tenant(
    ...
    with cash_scope_session as session:
        tenant_a = signup_tenant(
            session,
            {
                "business_name": "Movimientos A",
                "owner_name": "Owner A",
                "email": "movements-a@example.test",
                "password": "test-only-movements-a",
                "business_type": "taqueria",
            },
        )
        tenant_b = signup_tenant(
            session,
            {
                "business_name": "Movimientos B",
                "owner_name": "Owner B",
                "email": "movements-b@example.test",
                "password": "test-only-movements-b",
                "business_type": "taqueria",
            },
        )
        owner_a, owner_b = tenant_a["user"]["id"], tenant_b["user"]["id"]
        ...
        assert (
            session.scalar(sa.select(sa.func.count()).select_from(models.cash_movement_commands))
            == 0
        )
    ...
    try:
        with factory() as session:
            tenant_a = signup_tenant(
                session,
                {
                    "business_name": "HTTP Reportes A",
                    "owner_name": "Owner A",
                    "email": "http-reports-a@example.test",
                    "password": "test-only-http-reports-a",
                    "business_type": "taqueria",
                },
            )
            tenant_b = signup_tenant(
                session,
                {
                    "business_name": "HTTP Reportes B",
                    "owner_name": "Owner B",
                    "email": "http-reports-b@example.test",
                    "password": "test-only-http-reports-b",
                    "business_type": "taqueria",
                },
            )
            owner_a, owner_b = tenant_a["user"]["id"], tenant_b["user"]["id"]
    ...
    try:
        with factory() as session:
            tenant_a = signup_tenant(
                session,
                {
                    "business_name": "API Caja A",
                    "owner_name": "Propietaria A",
                    "email": "api-cash-a@example.test",
            ...
            tenant_b = signup_tenant(
                session,
                {
                    "business_name": "API Caja B",
                    "owner_name": "Propietario B",
                    "email": "api-cash-b@example.test",
        ...
        history = client.get(f"/api/v1/cash/shifts?branch_id={branch_a}", headers=headers_a)
