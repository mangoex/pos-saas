// In-memory product description editor fixture. No credentials or production services.
// node tests/browser/product-description-fixture.mjs
...
const description = 'Latte helado preparado al momento';
let products = [{id:'latte',name:'LATTE QA',sku:'LATTE',description,station:'kitchen',is_promo:false,promo_price_cents:null,promo_badge_text:null,image_url:null,category_id:'coffee',category_name:'Cafe',price_cents:4500,status:'active'}, {id:'plain',name:'OTHER QA',sku:'OTHER',category_id:'other',category_name:'Otra',price_cents:2000,status:'active'}];
  ...
  configFile: false, root: resolve('apps/admin-web'), plugins: [react(), {
    name: 'category-qa', configureServer(vite) {
      vite.middlewares.use(async (req, res, next) => {
        ...
        else if (req.url === '/api/v1/products/latte/simple-modifiers') body = {product:products[0],revision:'qa',editable:true,options:[]};
        else if (req.url === '/api/v1/catalog/products/latte' && req.method === 'PUT') {
          let raw = ''; for await (const chunk of req) raw += chunk;
          const {simple_modifiers, ...fields} = JSON.parse(raw);
          Object.assign(products[0],fields); body = products[0];
        }
...
await server.listen();
console.log('Admin QA: http://127.0.0.1:4177/qa-description');
