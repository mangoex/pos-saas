...
- `PRD-FR-096`: **Gestión de Plantillas HSM y SDK Nativo de Meta Embedded Signup**: Integración oficial del Meta JavaScript SDK (`FB.login`) para lanzamiento de popup interactivo de Embedded Signup sin manipulación manual de códigos, y gestión integral de Plantillas de Mensaje Pre-Aprobadas de Meta (HSM de Utilidad `restaurantos_order_update` y Marketing `restaurantos_reengagement_offer`), garantizando entrega de mensajes fuera de la ventana de 24 horas y cumplimiento con Meta Cloud API.
...
- `PRD-FR-738`: Suite Móvil de Administración y Puesta en Marcha Rápida en admin-web (Bottom Navigation Bar, Monitor de Pedidos con cobro y entrega integrada, Gestión de Turno de Caja, Catálogo Ágil y Disponibilidad con IA/Fotos, Configuración de Sucursal y Enlaces Compartibles). Al marcar un pedido como listo o entregado desde el monitor móvil, el sistema permite registrar y confirmar el cobro con el método de pago seleccionado contra el turno de caja abierto, asegurando que la venta compute en cortes y reportes. Los comandos de caja conservan la misma `Idempotency-Key` al reintentar un resultado incierto para el mismo payload y los movimientos manuales exigen una referencia de evidencia capturada por el operador; nunca se inventa una evidencia fija. Mientras admin-web permanece abierta, una intención pública nueva `PENDING_REVIEW` de la sucursal autorizada produce una sola alerta visible y audible incluso si el administrador navega por Caja, Menú o Sucursal; la carga inicial, el cambio de sucursal y la aceptación posterior no producen alertas falsas. Si el navegador suspende o bloquea el audio, la interfaz lo declara y conserva la alerta visual.
- `PRD-FR-739`: El comensal puede dictar o escribir una solicitud desde el menú web móvil para obtener un borrador editable del carrito. El asistente sólo propone productos, cantidades y opciones vigentes de la sucursal pública; Python reconcilia la salida contra el catálogo, toda selección obligatoria incompleta requiere decisión humana y el checkout canónico conserva autoridad exclusiva para precio, persistencia y creación de la intención pública.
...
- `PRD-NFR-531`: El pedido asistido público permanece apagado sin configuración explícita, limita entrada y frecuencia, redacta teléfonos y nombres expresados mediante patrones explícitos de identificación antes de la frontera OpenRouter, no registra transcripciones ni payloads completos y falla cerrado ante proveedor, salida o catálogo inválidos. La falta de reconocimiento de voz o permiso de micrófono nunca bloquea la captura escrita.
