...
- Detección responsiva en `apps/admin-web`: cuando el viewport es móvil (`width < 768px`) y no se encuentra forzada la vista desktop, se monta `MobileAdminShell` con barra de navegación inferior fija (`BottomNavigationBar`) con 4 pestañas operativas ergonómicas:
  ...
  2. **Caja (`cash`)**: Control de turno de caja (`MobileCashShiftTab`). Si está cerrada: formulario de apertura con fondo inicial en efectivo (`POST /cash/shifts/open`). Si está abierta: resumen en tiempo real de apertura, saldo y ventas, movimientos de caja (`POST /cash/movements`) y cierre operativo (`POST /cash/shifts/{id}/close-operationally`). Configuración de horarios semanales de servicio (L, M, M, J, V, S, D) con horas de apertura/cierre o días marcados como cerrado, apertura/cierre automático de caja (`reconcile_branch_auto_cash_shift`) con fondo inicial predeterminado configurable (por defecto $500.00 MXN), y configuración de métodos de cobro aceptados (Efectivo por defecto, Tarjeta con terminal, y Transferencia bancaria con Banco, Nombre, Cuenta y CLABE) que condicionan dinámicamente las opciones presentadas en el carrito del menú digital público. Preserva en todo momento la autoridad de apertura y cierre manual del administrador.
